...
public class FullbrightMain implements ClientModInitializer {
    ...
    public static void useOwnTicks() {
        ...
        if (mc.options.useKey.isPressed())
        {
            if (isLookingAt(Blocks.OBSIDIAN, generalLookPos().getBlockPos()) || isLookingAt(Blocks.BEDROCK, generalLookPos().getBlockPos())) {
                ...
                if (canPlaceServer(generalLookPos().getBlockPos())) {
    ...
    public static int limitPackets() {
        int stop = 2;
        if (getLatency() > 50) stop = 2;
        if (getLatency() < 50) stop = 1;
        return stop;
    }
    ...
    private static boolean canPlaceServer(BlockPos block) {
